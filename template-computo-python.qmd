...
$$ Given independent, identically distributed (i.i.d.) Monte Carlo draws, the Monte Carlo estimator $$
\widehat{I}_M \;\;=\;\; \frac{1}{M}\sum_{m=1}^{M} f(\theta^{(m)})
$$ converges almost surely to the true posterior expectation $\mathbb{E}[f(\Theta)\mid y]$ by the strong law of large numbers. Moreover, if $\textrm{var}[f(\Theta)\mid y] < \infty$, then a central limit theorem implies the asymptotic normal approximation $$
\sqrt{M}\Big(\widehat{I}_M - \mathbb{E}[f(\Theta)\mid y]\Big)
\;\xrightarrow[]{\;\;d\;\;}\;
\mathcal{N}\!\left(0,\;\textrm{var}[f(\Theta)\mid y]\right),
$$ which justifies the familiar $\mathcal{O}(1/\sqrt{M})$ Monte Carlo error rate and standard-error estimates for averages. Finite-sample concentration bounds are also hold under additional regularity assumptions (e.g., boundedness or sub-exponential tails of $f(\Theta)$), giving us explicit deviation probabilities for $\widehat{I}_M$ around $\mathbb{E}[f(\Theta)\mid y]$ [@gelman2013bda; @paulin2015concentration].
...
In addition, posterior simulations can be used for multiple imputation of missing data [@rubin1996multiplerubin] and to understand the fitted model. For example, in a logistic regression model, $Pr(y=1|x)=\mbox{logit}^{-1}(a + bx)$, one can make a scatterplot of posterior simulations of $(a,b)$, along with a graph of data $(x,y)_n, n=1,\dots,N$ along with curves, $\mbox{logit}^{-1}(a + bx)$ corresponding to those draws of $(a,b)$.
...
The idea of Markov chain Monte Carlo (MCMC) is that *correlated* draws from an ergodic Markov chain with invariant distribution $p(\theta\mid y)$ can still be used in the same sample-average estimator $\widehat{I}_M$. Under standard conditions (e.g., Harris ergodicity) that are easily satisfied in practical applications, the Markov chain ergodic theorem guarantees $\widehat{I}_M \to \mathbb{E}[f(\Theta)\mid y]$ even though the draws are dependent [@roberts2004mcmc].
...
$$ where $\pi(\cdot)=p(\cdot\mid y)$, $\tau_\text{int}=1+2\sum_{k\ge 1}\textrm{corr}_\pi\!\big(f(\Theta_0), f(\Theta_k)\big)$ and $\{\Theta_k\}$ denotes the stationary chain. This formula for the variance motivates the usual definition of the effective sample size $M_\text{eff}=M/\tau_\text{int}$ and the heuristic $\mathcal{O}(1/\sqrt{M_\text{eff}})$ error rate [@roberts2004mcmc; @vehtari2021rank]. Under similarly strong assumptions (e.g., uniform or geometric ergodicity, or a spectral-gap condition), one can also obtain non-asymptotic deviation bounds and Bernstein/Hoeffding-type concentration inequalities for Markov chain averages [@paulin2015concentration].

Unfortunately, these strong conditions needed for MCMC central limit theorems and concentrations bounds are typically difficult to verify for the complex MCMC methods and posteriors encountered in modern Bayesian inference over continuous parameter spaces [@roberts2004mcmc]. Even so, MCMC, especially modern gradient-based variants, has shown remarkable empirical success in applied Bayesian inference, with practical reliability assessed via convergence diagnostics, effective sample sizes, and predictive checks rather than direct verification of theoretical assumptions [@gelman2020workflow; @vehtari2021rank].
...
We will assume a very simple multivariate regression formulation with an intercept, $P \in \mathbb{N}$ covariates, and $N \in \mathbb{N}$ observations. Our data is made up of observations $y_n \in \mathbb{R}$ paired with covariate vectors $x_n \in \mathbb{R}^{P}$, assembled into a vector $y \in \mathbb{R}^N$ and a design matrix $x \in \mathbb{R}^{N \times P}$. We will assume the usual parameters consisting of an intercept $\alpha \in \mathbb{R}$, regression coefficients (aka slopes) $\beta \in \mathbb{R}^P$, and an error scale $\sigma \in (0, \infty)$. We will assume the data and covariates are approximately on unit scale so that we can consider the following prior as weakly informative: $$
...
$$ The joint density defining our Bayesian model (with data $x$ taken as an unmodeled constant) is thus $$\textstyle
\begin{array}{rcl}
...
\log p^\textrm{unc}(\alpha, \beta, \sigma^\textrm{unc} \mid x, y)
...
``` stan
{{< include linear-regression.stan verbatim=true >}}
```
...
The parameter values used for the simulation are as follows.
...
```{python python-code}
#| code-fold: false
fit = m.sample(data=data, show_progress=False, seed=145777)
```
...
The `summary_csp` object is a pandas `DataFrame` containing the same table printed above. Additional run metadata and diagnostics beyond the table (configuration, console outputs, tuning parameters, etc.) live on `fit`, not in `summary_csp`. The variable `lp__` reported in the first row is the total log probability density (log posterior up to an additive constant). The variables `alpha`, `beta`, and `sigma` are model parameters, and `y_new` reports the model’s posterior predictive draws from the generated quantities block. The remaining columns include the posterior mean, Monte Carlo standard error, posterior standard deviation, median absolute deviation (MAD), three quantiles (5%, 50%, and 95%), and convergence diagnostics including bulk and tail effective sample size and rank-normalized split $\widehat{R}$; definitions are provided by @vehtari2021rank.
...
-   *Functions block*: This block allows the user to define functions for use throughout the rest of the program (such as in `transformed data`, `model`, and `generated quantities`). Each function is translated to a C++ function templated to support automatic differentiation. User-defined functions are not closures: they cannot access variables declared in other blocks except via their arguments. Arguments and return values may be scalars, vectors, matrices, and arrays of these types. Stan does not support user-defined higher-order functions in the sense that user-defined functions cannot take other functions as arguments (the ODE solvers are special built-ins for this reason).

-   *Data and transformed data blocks*: These blocks specify the types of the (known) data and allow the user to perform data-preprocessing. Each data declaration in the data block is translated to an instance variable of the generated class. The data block can only contain declarations, and thus behaves like the signature for the data ingestion function. The transformed data block is executed after the data is read in as the model object is being constructed. After the model object is constructed with the data, it remains immutable to allow safe multi-threaded application.

-   *Parameter and transformed parameter blocks*: These blocks specify the types of the (unknown) parameters and allow the user to perform transmations of the parameters that they would also like to obtain posterior draws for. The log density function is defined for unconstrained inputs gathered into a vector. The first part of the function unpacks the entries of that vector, applies the constraining transform to define constrained variables locally, then adds the log Jacobian determinant to the Jacobian accumulator to adjust for the change-of-variables making up the constraint. Transformed parameters also define local variables. Everything executes serially as coded in the Stan program.
...
&= \log \textrm{normal}(\alpha \mid 0, 5)
 + \sum_{p=1}^P \log \textrm{normal}(\beta_p \mid 0, 2.5) \\
&\quad + \log \textrm{exponential}(\sigma \mid 0.5)
 + \sum_{n=1}^N \log \textrm{normal}(y_n \mid \alpha + x_n \cdot \beta, \sigma).
\end{aligned}
...
``` cpp
template <bool propto, bool jacobian, typename T>
inline T log_prob(Vector<T>& params) const {
  Accumulator<T> accum;
  ...
  auto sigma = in.template read_constrain_lb<T, jacobian>(0, log_jacobian);
  ...
  accum.add(normal_lpdf<propto>(y, add(alpha, multiply(x, beta)), sigma));
...
Before we begin, we will also convert the Python and NumPy data arrays to JAX.
...
## The unnormalized log posterior in JAX
...
def log_posterior(params):
    ...
    # Stan: exponential(0.5) uses rate=0.5  =>  scale = 1 / 0.5
...
Before we can use this unnormalized log posterior for sampling, we need a few support functions. To mirror Stan, the sampler needs to work on the transformed scale. Here, as in most programs, the transform maps constrained variables to unconstrained ones. In our regression example, the scale, which is constrained to $(0, \infty)$, is log-transformed so that the transformed scale ranges over the whole real number line, $(-\infty, \infty)$.
...
## The transformed log density in JAX

The transformed (unnormalized) log posterior function we will use for sampling can be defined to apply the inverse transform and add the adjustment.
...
Following Stan, we need a way to generate a transformed random initialization. Here we use JAX's version of NumPy's `numpy.random` library (imported above as `jrd`).
To match Stan's default initialization, we generate transformed initial values on the unconstrained scale from `uniform(-2, 2)`.
...
def random_init_transformed(key):
    ...
    t_alpha = jrd.uniform(key0, minval=-2.0, maxval=2.0)
    t_beta = jrd.uniform(key1, shape=(2,), minval=-2.0, maxval=2.0)
    t_sigma = jrd.uniform(key2, minval=-2.0, maxval=2.0)
    t_params = { "alpha": t_alpha, "beta": t_beta, "sigma": t_sigma }
    return t_params
```
...
seed = 441_582
key = jrd.key(seed)
init_key, nuts_key, gq_key = jrd.split(key, 3)
t_params_init = random_init_transformed(init_key)
print(f"{t_params_init=}")
```

As a consistency check, we verify that the parameters we used to initialise the sampler are round-trippable through the transform and inverse transform.
...
Before sampling, we first define a top-level Markov chain Monte Carlo sampler, and then we instantiate it with the NUTS transition kernel configured to run in a way that matches Stan's defaults.
...
results = [] 
for i in range(N): 
    params_i = jax.tree.map(lambda a: a[i], draws) 
    results.append(posterior_predictive(keys[i], params_i)) 

# list-of-pytrees -> pytree-of-stacked-arrays (matches vmap out_axes=0) 
return jax.tree.map(lambda *xs: jnp.stack(xs), *results)
```

The specification `in_axes=(0, 0)` tells `vmap` to iterate over axis 0 of both arguments: over `keys[i]` and over the leading axis of each array in the `draws` PyTree.
...
1.  There are no data declarations. The functions simply bind (i.e., close over) the data variables `x` and `y` (i.e., they read them from the environment where they were defined); we will shortly show how to abstract this into a function.
...
5.  There are no function blocks because plain old Python functions can be applied to JAX objects. The main caveat is that these functions will be traced: under `jax.jit`, ordinary Python `if` and `while` statements cannot branch on traced parameter values. When value-dependent control flow is needed, it should be written with JAX control-flow primitives such as `jax.lax.cond` or `jax.lax.while_loop`.
...
def linear_regression(x, y, x_new):
    ...
    def log_density(a, b, s):
        ...
        lp += normal(a, 0, 5)
        lp += normal(b, 0, 2.5)
        # Match Stan's exponential(0.5), where 0.5 is the rate, so scale = 1 / 0.5 = 2
        lp += exponential(s, 2)
        lp += normal(y, a + x @ b, s)
        return lp
...
After calling this function with data, the resulting `model` object is feature equivalent to all the manual code written above for transforming parameters and performing initialization. It mirrors the C++ object produced by Stan (also the model object produced by BridgeStan [@roualdes2023bridgestan]). Because these simple functions generate fully JAX-embedded code, the log density function can be automatically differentiated and all of the functions can be just-in-time compiled.
...
In order to automate the transformation functionality that were coded manually above, we introduce the idea of a "parameter specification" (the `parameters` dictionary in the code) which provides all the information about how the parameters should be transformed. The leaves of this parameter specification are simple classes like `positive`, which is reproduced here in full:
...
The densejax package provides a menu of existing transforms, but the above class is designed to be simple enough to be easily extensible.

The `real` class is simply a transform that has the identity for both sides of the transformation; we rely on JAX's jit compiler to remove any overhead from this implementation style.

Turning this parameter spec into functions that perform the inverse transforms, transforms, and (possibly random) initialization is trivial. All of these reduce to simple map-like operations over the specification and the provided parameter values where required.

The majority of the code in densejax consists of these transformation classes, utility functions consuming them, and the small wrappers provided for common distributions like `normal` as seen above.

## Simplifying inference

The `model` object remains general enough for use in any gradient based sampler implemented in JAX; the one-liner

``` python
regression_model = linear_regression(x, y, x_new)
log_posterior_transformed = jax.jit(regression_model.log_density)
```

would be enough to use `blackjax` in the same manner as above.

However, bundling more of the desired functionality into one object allows for more turn-key helpers. Consider the following new definition of `nuts_sample`:
...
This version of `nuts_sample` has the advantage over the previous that both the input initial values *and the returned draws* are in the model space. The user on the outside does not need to worry about transforming the initial values and inverse-transforming the results.

We could have written `nuts_sample` to do this for us before, if we had been willing to pass it additional arguments; the `model` class makes it very natural to define the most useful version out of the box.

Given these snippets, one can recreate the complete sampling workflow from the prior section with the following brief code:
...
sample_key, gq_key = jrd.split(key)
draws = nuts_sample(sample_key, regression_model, num_draws)

mean = functools.partial(jax.tree.map, functools.partial(jnp.mean, axis=0))
std = functools.partial(jax.tree.map, functools.partial(jnp.std, axis=0))

print(f"{mean(draws)=}")
print(f"{std(draws)=}")

# posterior predictive
gq_keys = jrd.split(gq_key, num_draws)
pred_draws = jax.vmap(regression_model.forward, in_axes=(0, 0))(gq_keys, draws)

print(f"{mean(pred_draws)=}")
print(f"{std(pred_draws)=}")
```
