...
2.  JAX does not require parameters to be serialized. With the wizardry of PyTree, the Blackjax sampling package can work directly with the `log_prob` function without any need for serialization of the parameters into an array.  If serialized log density functions are needed elsewhere, they are convertible with a single function call as we show below.
...
Aside form the verbosity of all the namespace qualifiers and the need for all the intermediate calls to `sum`, the biggest obstacle to writing code this way is having to manually deal with the constraining and unconstraining transform.
