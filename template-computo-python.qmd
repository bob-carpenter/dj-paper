...
In practice, exactly independent draws from $p(\theta\mid y)$ are rarely available. Fortunately, many less-demanding sampling schemes still give estimators that converge to the correct posterior expectations.
These include rejection sampling; importance sampling (which uses weights); sequential Monte Carlo (which propagates and reweights a particle population); and Markov chain Monte Carlo (which constructs a dependent sequence whose stationary distribution is the posterior) [@gelman2013bda; @doucet2001introduction; @roberts2004mcmc].
