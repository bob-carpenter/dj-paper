...
Here's an example Stan program defining a linear regression, which we have placed into the file `examples/regression.stan`. 
```stan
data {
...
```{python python-code}
#| code-fold: false
m = csp.CmdStanModel(stan_file="examples/regression.stan")
```
