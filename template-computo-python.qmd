...
This statement accumulates, term by term, the log density derived above. The intercept prior `normal_lupdf(alpha | 0, 5)` contributes $\log \textrm{normal}(\alpha \mid 0, 5)$; the
coefficient and observation statements are vectorized, so the single calls `normal_lupdf(beta | 0, 2.5)` and `normal_lupdf(y | alpha + x * beta, sigma)` evaluate to the sums $\sum_{p=1}^P \log
\textrm{normal}(\beta_p \mid 0, 2.5)$ and $\sum_{n=1}^N \log \textrm{normal}(y_n \mid \alpha + x_n \cdot \beta, \sigma)$, respectively. The products over $p$ and $n$ in the joint density
thus become summations on the log scale, accumulated into target. The one term with no corresponding statement is the Jacobian adjustment $\sigma^\textrm{unc}$: Stan adds it automatically
