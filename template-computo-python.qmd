...
$$
\sigma_f^2
= \textrm{var}_\pi\!\big(f(\Theta)\big)
  + 2\sum_{k=1}^{\infty}\textrm{cov}_\pi\!\big(f(\Theta_0), f(\Theta_k)\big),
$$
where $\pi(\cdot)=p(\cdot\mid y)$ and $\{\Theta_k\}$ denotes the stationary chain. Equivalently, $\sigma_f^2=\textrm{var}_\pi(f)\,\tau_\text{int}$, where $\tau_\text{int}=1+2\sum_{k\ge 1}\rho_k$ is the integrated autocorrelation time, motivating the usual definition of an effective sample size $M_\text{eff}=M/\tau_\text{int}$ and the heuristic ``$\mathcal{O}(1/\sqrt{M_\text{eff}})$'' error rate [@roberts2004mcmc; @vehtari2021rank].
